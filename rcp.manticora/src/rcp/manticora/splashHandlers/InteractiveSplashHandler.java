...
package rcp.manticora.splashHandlers;
...
// ************************* Progress label y bar *************************
	...
	*/
	...
	private void handleButtonOKWidgetSelected() {
	    ...
	    // si no pasa las validaciones, salimos sin ejecutar el ciclo del ok
	    if (!validarCampos()) {
	    	return;
	    } else {
	    	// si el campo de servidor está habilitado, actualizar el PreferenceStore
	    	if (fTextServidor.getEnabled()) {
				...
				store.setValue("schema", fTextSchema.getText());
				try {
					store.save();
				} catch (IOException e) {
					MessageDialog.openError(getSplash(), "Error en preferencias", "No se ha podido guardar las preferencias.");
					e.printStackTrace();
					return;
				}
	    	}
...
//****************************** Progress bar *****************************
	...
	private void createUITextServidor() {		
		...
		if (s.equals("servidor ")) {
			fTextServidor.setEnabled(true);
			createUILabelTextSchema();
		} else {
	...
	private void createUILabelTextSchema() {
		// Create the label
		Label label = new Label(fCompositeLogin, SWT.NONE);
		label.setText("Sc&hema:"); //NON-NLS-1
		// Configure layout data
		GridData data = new GridData();
		data.horizontalIndent = F_LABEL_HORIZONTAL_INDENT;
		label.setLayoutData(data);
		...
		// Create the text widget
		fTextSchema = new Text(fCompositeLogin, SWT.BORDER);
		// Configure layout data
		data = new GridData(SWT.NONE, SWT.NONE, false, false);
		data.widthHint = F_TEXT_WIDTH_HINT;
		data.horizontalSpan = 2;
		fTextSchema.setLayoutData(data);
		fTextSchema.setText("schema");
	}
